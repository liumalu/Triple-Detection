...
检测页面布局：顶部任务选择 + 左侧图像显示 + 右侧任务信息和控制 + 底部操作日志
...
│                                 │   ─────────────────           │
│                                 │   产品：OCR 产品 A            │
│   [VmRenderControl              │   批次：2026052501            │
│    VisionMaster                │   生产日期：2026-05-25        │
│    显示区域]                    │   有效期至：2027-05-25        │
│                                 │                              │
│        640 x 480                │   方案操作                    │
│                                 │   ─────────────────           │
│                                 │   [选择方案] [加载方案]       │
...
│   操作日志                                                     │
│   ┌────────────────────────────────────────────────────────┐  │
│   │ 12:30:15 [执行检测] Task-001 开始执行                   │  │
│   │ 12:30:20 [执行检测] Task-001 检测完成，结果: OK          │  │
│   │ 12:30:25 [系统] 方案已保存                               │  │
│   └────────────────────────────────────────────────────────┘  │
└────────────────────────────────────────────────────────────────┘
```
...
**布局说明：**
- **顶部**：任务下拉选择
- **左侧**：VisionMaster 图像显示区域 (主要区域)
- **右侧**：
  - 任务信息卡片（产品、批次、生产日期、有效期）
  - 方案操作（选择方案 + 加载方案）
  - 检测结果统计（OK/NG 数量、置信度、耗时）
  - 控制按钮（启动/停止/重置）
- **底部**：实时操作日志滚动区域

**控制按钮：**
- [选择方案] - 打开文件对话框选择 .sol 方案文件
- [加载方案] - 加载选中的方案文件
- [启动] - 开始执行检测（运行时按钮变为 [停止]）
- [停止] - 停止检测
- [重置] - 清除结果
...
- [x] 检测页面：左侧图像 + 右侧任务信息/方案操作/检测结果/控制按钮 + 底部日志
