...
namespace TripleDetection.Services.Production
{
    ...
    public class TaskService : ITaskService
    {
        ...
        public TaskService() : this(new SqliteRepositoryFactory().CreateRepository<ProdTask>(), null)
        {
        }

        public TaskService(IRepository<ProdTask> repository, IAuditLogService auditLogService)
        {
            _repository = repository;
            _auditLog = auditLogService;
        }

        public IEnumerable<ProdTask> GetAll()
        {
            return _repository.GetAll();
        }

        public IEnumerable<ProdTask> GetByStatus(TaskStatus status)
        {
            return _repository.Find(x => x.Status == status);
        }

        public ProdTask GetById(int id)
        {
        ...
        public IPagedResult<ProdTask> Query(PagedQuery query)
        {
            return _repository.Query(query);
        }

        public IPagedResult<ProdTask> Query(TaskQuery query)
        {
            return _repository.Query(query);
        }
    }
