...
namespace TripleDetection.Services
{
    ...
    public class UserService : IUserService
    {
        ...
        public UserService() : this(new InMemoryRepository<User>())
        {
        }
        ...
        {
            var user = _repository.Find(x => x.Username == username && !x.IsDeleted);
            if (user != null)
            {
                foreach (var u in user)
                {
                    if (ValidatePassword(u.PasswordHash, password))
                    {
                        return u;
                    }
                }
        ...
        {
            // 简单验证，生产环境应使用 BCrypt 或 Argon2
            return passwordHash == HashPassword(password);
        }

        public string HashPassword(string password)
        {
            // 简单哈希，生产环境应使用 BCrypt 或 Argon2
